...
export class FinanceReportService {
  ...
  async getOverview() {
    ...
    const refunds = returns.reduce(
      (sum, ret) => sum + ret.order.finalAmount,
      0,
    );
    ...
    const prevEntries = entries.filter(
      (e) => e.createdAt >= sixtyDaysAgo && e.createdAt < thirtyDaysAgo,
    );
    ...
    const prevExpenses = prevEntries.reduce(
      (sum, entry) => sum + entry.amount,
      0,
    );
  ...
  async getMonthlyBreakdown() {
    ...
    const breakdown = monthNames.map((month, index) => {
      const monthOrders = orders.filter(
        (o) => o.createdAt.getMonth() === index,
      );
      const monthEntries = entries.filter(
        (e) => e.createdAt.getMonth() === index,
      );
      ...
      const refunds = monthReturns.reduce(
        (sum, ret) => sum + (ret.order?.finalAmount || 0),
        0,
      );
