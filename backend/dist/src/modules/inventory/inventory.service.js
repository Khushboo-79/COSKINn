...
let InventoryService = class InventoryService {
    ...
    async createWarehouse(dto) {
        return this.prisma.warehouse.create({
            data: {
                name: dto.name,
                code: dto.name.toUpperCase().replace(/\s+/g, '-').substring(0, 10) +
                    '-' +
    ...
    async stockOut(dto) {
        ...
        return this.prisma.$transaction(async (prisma) => {
            ...
            if (!stock || stock.quantity < dto.quantity) {
                throw new common_1.BadRequestException(`Insufficient stock for SKU ${dto.sku}`);
            }
            const movement = await prisma.stockMovement.create({
                data: {
                    warehouseId: dto.warehouseId,
    ...
    async adjustStock(dto) {
        ...
        return this.prisma.$transaction(async (prisma) => {
            ...
            if (dto.quantity < 0 &&
                ...
                throw new common_1.BadRequestException(`Insufficient stock for adjustment of SKU ${dto.sku}`);
            }
            ...
            await this.auditService.logEvent('ADJUST_STOCK', 'InventoryStock', updatedStock.id, 'SYSTEM', { prevQuantity: stock?.quantity || 0 }, { newQuantity: updatedStock.quantity, reason: dto.reason });
    ...
    async reserveStock(sku, quantity, tx = this.prisma) {
        const stocks = await tx.inventoryStock.findMany({
            where: { sku, quantity: { gte: quantity } },
            orderBy: { quantity: 'desc' },
        });
        if (stocks.length === 0) {
            throw new common_1.BadRequestException(`Insufficient stock to reserve SKU ${sku}`);
        }
    ...
    async deductReservedStock(sku, quantity, tx = this.prisma) {
        const stocks = await tx.inventoryStock.findMany({
    ...
    async releaseReservedStock(sku, quantity, tx = this.prisma) {
        const stocks = await tx.inventoryStock.findMany({
            where: { sku, reservedQty: { gte: quantity } },
        });
        if (stocks.length === 0)
