...
import { Package, Search, Plus, Filter, Edit2, Trash2, X, Layers, Tags, Shield, Image as ImageIcon, FileText, Send, PowerOff, Globe } from 'lucide-react';
...
import MarketingFeedPreview from './MarketingFeedPreview';
...
export default function ProductManager() {
  ...
  const [managingMediaFor, setManagingMediaFor] = useState<any | null>(null);
  const [managingComplianceFor, setManagingComplianceFor] = useState<any | null>(null);
  ...
  const [isViewingMarketingFeed, setIsViewingMarketingFeed] = useState(false);
  ...
  const { data: products, isLoading } = useQuery({
    queryKey: ['products', filterCategory, searchTerm],
    queryFn: async () => {
  ...
  const createMutation = useMutation({
    mutationFn: (newProduct: ProductFormValues) => api.post('/product', newProduct),
    onSuccess: () => {
  ...
  const submitApprovalMutation = useMutation({
    mutationFn: (id: string) => api.post(`/product/${id}/submit-approval`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['products'] });
  ...
  const deactivateMutation = useMutation({
    mutationFn: (id: string) => api.post(`/product/${id}/deactivate`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['products'] });
      alert('Product deactivated and moved to draft.');
    },
  });
  ...
  if (managingMediaFor) {
    return <MediaManager 
      productId={managingMediaFor} 
      onClose={() => setManagingMediaFor(null)} 
    />;
  }

  if (managingComplianceFor) {
    const productData = products?.find((p: any) => p.id === managingComplianceFor);
    return <ComplianceManager 
      product={productData}
      onClose={() => setManagingComplianceFor(null)}
      onRefresh={() => queryClient.invalidateQueries({ queryKey: ['products'] })}
    />;
  }

  if (managingContentFor) {
    const productData = products?.find((p: any) => p.id === managingContentFor);
    return <ContentManager 
      product={productData}
      onClose={() => setManagingContentFor(null)}
      onRefresh={() => queryClient.invalidateQueries({ queryKey: ['products'] })}
    />;
  }

  if (selectedSeo) {
    return <SeoManager product={selectedSeo} onClose={() => setSelectedSeo(null)} />;
  }

  if (isViewingMarketingFeed) {
    return (
      <div className="space-y-6">
        <button 
          onClick={() => setIsViewingMarketingFeed(false)}
          className="text-sm font-medium text-slate-500 hover:text-slate-700 flex items-center gap-1"
        >
          ← Back to Catalog
        </button>
        <MarketingFeedPreview />
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 bg-slate-100 text-slate-700 px-4 py-2 rounded-lg hover:bg-slate-200 transition-colors shadow-sm font-medium border border-slate-200"
          >
            Bulk Import / Export
          </button>
          <button
            onClick={() => setIsViewingMarketingFeed(true)}
            className="flex items-center gap-2 bg-indigo-50 text-indigo-600 px-4 py-2 rounded-lg hover:bg-indigo-100 transition-colors shadow-sm font-medium"
          >
            Marketing Feed
          </button>
          <button
            onClick={() => setIsCreating(true)}
            className="flex items-center gap-2 bg-rose-500 text-white px-4 py-2 rounded-lg hover:bg-rose-600 transition-colors shadow-sm font-medium"
          >
            ...
            <tbody className="divide-y divide-slate-100">
              {products?.map((product: any) => (
                <tr key={product.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    {product.status === 'DRAFT' && <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-slate-100 text-slate-600">Draft</span>}
                    {product.status === 'PENDING_APPROVAL' && <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-amber-100 text-amber-700 animate-pulse">Pending Review</span>}
                    {product.status === 'LIVE' && <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-emerald-100 text-emerald-700">Live</span>}
                    {product.status === 'REJECTED' && <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-red-100 text-red-700">Rejected</span>}
                    {product.rejectionReason && product.status === 'REJECTED' && (
                      <div className="text-[10px] text-red-500 mt-1 max-w-[120px] truncate" title={product.rejectionReason}>
                    ...
                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      {(product.status === 'DRAFT' || product.status === 'REJECTED') && (
                        <button 
                          ...
                          onClick={() => submitApprovalMutation.mutate(product.id)}
                      ...
                      {product.status === 'LIVE' && (
                        <button 
                          ...
                          onClick={() => {
                            if(confirm('Are you sure you want to deactivate this product? It will be removed from the store.')) {
                      ...
                      <button 
                        title="Compliance & Stock"
                        onClick={() => setManagingComplianceFor(product.id)}
                      ...
                      <button 
                        ...
                        onClick={() => setManagingContentFor(product.id)}
                      ...
                      <button 
                        title="Manage Media"
                        onClick={() => setManagingMediaFor(product.id)}
                      ...
                      <button 
                        onClick={() => setSelectedSeo(product)}
                        className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors"
                        title="Manage SEO Fields"
                      >
