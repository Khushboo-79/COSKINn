...
export const VideoTutorialsScreen = () => {
  ...
  const { data: videos = [], isLoading } = useQuery({
    ...
    queryFn: () => contentApi.getVideos()
  });

  const createVideoMutation = useMutation({
    mutationFn: (data: any) => contentApi.createVideo(data),
    onSuccess: () => {
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div className="p-8 border-b border-slate-200 bg-slate-50">
          <div className="border-2 border-dashed border-[#FF7F50]/30 rounded-2xl p-12 bg-white flex flex-col items-center justify-center relative overflow-hidden group hover:border-[#FF7F50]/50 transition-colors">
            
            {uploading ? (
              <div className="text-center space-y-4 w-full max-w-sm">
                <Loader2 className="h-10 w-10 text-[#FF7F50] animate-spin mx-auto" />
                <div>
                ...
                <div className="w-full bg-slate-100 rounded-full h-2 mt-4 overflow-hidden">
                  <div 
                    className="bg-[#FF7F50] h-2 rounded-full transition-all duration-300"
              ...
              <>
                <div className="bg-[#FF7F50]/10 p-4 rounded-full mb-4 group-hover:scale-110 transition-transform">
                  <Upload className="h-8 w-8 text-[#FF7F50]" />
                </div>
                ...
                <button className="mt-6 px-6 py-2 bg-[#FF7F50] text-white rounded-lg font-medium hover:bg-[#FF7F50]/90 pointer-events-none">
                  Select File
                </button>
              </>
            )}
        ...
        <div className="p-8">
          <h3 className="font-bold text-slate-900 mb-6 flex items-center">
            <Video className="h-5 w-5 mr-2 text-slate-400" /> Uploaded Videos ({videos.length})
          </h3>
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {videos.map((video: any) => (
                <div key={video.id} className="border border-slate-200 rounded-xl p-4 flex gap-4 bg-white shadow-sm hover:border-[#FF7F50]/30 transition-colors group">
                  ...
                  <div className="flex-1 flex flex-col min-w-0">
                    <h4 className="font-bold text-slate-900 text-sm truncate" title={video.title}>{video.title}</h4>
                    ...
                    <div className="mt-auto flex gap-2 pt-3">
                      <button 
                        ...
                        className="flex-1 text-xs font-medium text-[#FF7F50] bg-[#FF7F50]/10 hover:bg-[#FF7F50]/20 py-1.5 rounded flex items-center justify-center transition-colors"
                      >
                      ...
                      >
                        {deleteVideoMutation.isPending ? <Loader2 className="h-3 w-3 animate-spin" /> : <Trash2 className="h-3 w-3" />}
                      </button>
                    </div>
                  </div>
                </div>
