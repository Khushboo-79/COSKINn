...
export default function AuthModal({ isOpen, onClose }) {
  const { sendMobileOtp, verifyMobileOtp, checkMobileExists, checkEmailExists, loginWithMobile, loginWithEmail, authenticateOTPUser } = useAuth();
  const { theme } = useTheme();
  
  const [step, setStep] = useState('MOBILE'); // MOBILE, EMAIL_LOGIN_INPUT, OTP, DETAILS, EMAIL_OTP
  const [mobile, setMobile] = useState('');
  const [emailLogin, setEmailLogin] = useState('');
  const [otp, setOtp] = useState(['', '', '', '', '', '']); // 6 digits for mobile OTP
  const [name, setName] = useState('');
  const [email, setEmail] = useState('');
  const [emailOtp, setEmailOtp] = useState(['', '', '', '', '', '']); // 6 digits for email OTP
  ...
  const handleOtpChange = (index, value, isEmail = false) => {
    ...
    const maxLength = 6;
    if (value.length > 1) {
      const pastedData = value.slice(0, maxLength).split('');
                  ...
                  {duplicateEmailError && (
                    <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-start gap-2 mt-3 text-red-500 text-sm font-medium bg-red-50 p-3 rounded-xl border border-red-100">
                      ...
                      <span className="leading-tight">
                        ...
                        <button type="button" onClick={() => { setStep('EMAIL_LOGIN_INPUT'); setEmailLogin(email); setErrorMsg(''); }} className="underline font-bold hover:text-red-700">sign in</button>
                        {' '}instead.
                      </span>
                    </motion.div>
                  )}
