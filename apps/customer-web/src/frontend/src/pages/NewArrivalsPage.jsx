...
export default function NewArrivalsPage() {
  ...
  const filters = ['All', 'Skincare', 'Cosmetics', 'Newest', 'Price', 'Popular', 'Availability'];
  ...
  const justDropped = products.slice(0, 5);
  ...
  const filteredProducts = activeFilter === 'All' ? products : 
    (activeFilter === 'Skincare' || activeFilter === 'Cosmetics') ? products.filter(p => p.category === activeFilter) : 
    products;
              ...
              >
                ...
                <div className="relative w-full aspect-square rounded-2xl overflow-hidden mb-6 bg-theme-secondary/20">
                  ...
                  <div className="absolute inset-0 bg-black/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center backdrop-blur-[2px]">
                    <button className="px-6 py-3 bg-white text-black text-xs font-bold uppercase tracking-widest rounded-full hover:bg-theme-primary hover:text-white transition-colors duration-300 flex items-center gap-2 transform translate-y-4 group-hover:translate-y-0 shadow-xl">
                ...
                <div className="flex flex-col flex-1">
                  <div className="flex items-center gap-1 mb-2 text-yellow-400">
                    ...
                    <span className="text-xs text-black/60 font-medium ml-1">{product.rating}</span>
                  </div>
                  ...
                  <p className="text-sm text-black/50 line-clamp-2 mb-6 font-body">Luxurious formula enriched with premium ingredients for a flawless finish.</p>
              ...
              >
                ...
                <div className="px-4 pb-4">
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="font-heading font-bold text-lg md:text-xl leading-tight max-w-[80%]">{item.name}</h3>
                    <span className="font-bold text-lg text-theme-primary">₹{item.price}</span>
                  </div>
                  <button 
                    onClick={() => addToCart(item)}
                    className="mt-4 flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-black/60 group-hover:text-theme-primary transition-colors"
                  >
          ...
          {[1, 2, 3].map((item, idx) => (
            ...
            >
              ...
              <div className="flex-1">
                ...
                <h4 className="font-heading font-bold text-lg mb-3">{idx === 1 ? 'COSKINn Magnetic Lipstick' : 'COSKINn Blueberry Overnight Mask'}</h4>
        ...
        <div className="flex gap-8 overflow-x-auto hide-scrollbar pb-12 snap-x snap-mandatory px-4">
          {[1, 2, 3, 4, 5].map((item, idx) => (
            <div key={item} className="snap-center shrink-0 w-[280px] md:w-[320px] bg-white rounded-3xl p-6 shadow-sm border border-black/5 hover:shadow-xl transition-shadow duration-300 flex flex-col items-center text-center group">
              ...
              <h4 className="font-heading font-bold text-lg mb-1">{idx % 2 === 0 ? 'COSKINn Strawberry Glow Cleanser' : 'COSKINn Magnetic Lipstick'}</h4>
              <p className="text-sm text-black/50 mb-6">{(482 + idx * 13).toLocaleString()} Reviews</p>
              <button 
                onClick={() => addToCart({
                  id: 100 + idx, 
                  name: idx % 2 === 0 ? 'COSKINn Strawberry Glow Cleanser' : 'COSKINn Magnetic Lipstick',
                  price: idx % 2 === 0 ? 499 : 999,
                  image: idx % 2 === 0 ? cleanserImg : lipstickImg
                })}
