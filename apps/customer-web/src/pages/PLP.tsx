  ...
  const handleFilterChange = (sectionId: string, option: string) => {
    setSelectedFilters(prev => {
      ...
      if (current.includes(option)) {
        return { ...prev, [sectionId]: current.filter(item => item !== option) };
      } else {
  ...
  const filteredProducts = allProducts.filter(product => {
    ...
    if (selectedFilters['Price'] && selectedFilters['Price'].length > 0) {
      const priceMatches = selectedFilters['Price'].some(range => {
        if (range === 'Under 2500') return product.price < 2500;
      ...
      <div className="max-w-[1400px] mx-auto px-6 lg:px-10 py-12">
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <div className="lg:hidden flex justify-between items-center mb-6">
            <p className="font-bold text-gray-500">{filteredProducts.length} products</p>
            <button 
                    ...
                    {product.badges && product.badges.length > 0 && (
                      <div className="absolute top-3 left-3 z-10 flex flex-col gap-1">
                        {product.badges.map(badge => (
                          <span key={badge} className={`px-2 py-1 text-[10px] font-bold uppercase tracking-wider rounded-sm shadow-sm ${isGlam ? 'bg-[#2a2a2a] text-[#e5b376]' : 'bg-[#ff9aa8] text-white'}`}>
                            {badge}
                          </span>
                        ))}
                      </div>
      ...
      <AnimatePresence>
        {toastMessage && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 50 }}
            className={`fixed bottom-8 left-1/2 transform -translate-x-1/2 z-50 px-6 py-3 rounded-full shadow-2xl flex items-center gap-3 font-bold text-sm ${
