...
const ShopByCategory: React.FC = () => {
  ...
  return (
    <section className={`py-20 ${isGlam ? 'bg-[#faf9f6]' : 'bg-[#fcfaf9]'}`}>
      <div className="max-w-[1400px] mx-auto px-6 lg:px-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16">
          ...
          <Link to="/collections" className="group flex items-center text-[13px] font-bold text-[#6b6b6b] hover:text-[#2a2022] transition-colors mt-6 md:mt-0 self-start md:self-auto">
            View all 
        ...
        {/* Categories Grid */}
        <div className="flex overflow-x-auto snap-x snap-mandatory md:grid md:grid-cols-5 gap-4 md:gap-8 lg:gap-12 pb-8 md:pb-0 hide-scrollbar -mx-6 px-6 md:mx-0 md:px-0">
          {categories.map((category, idx) => (
            <Link to={`/collections/${category.name.toLowerCase()}`} key={idx} className="flex-shrink-0 w-[60%] sm:w-[40%] md:w-auto snap-start flex flex-col items-center group cursor-pointer">
